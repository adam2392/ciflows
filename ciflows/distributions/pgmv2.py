...
class LinearGaussianDag(MultidistrCausalFlow):
    def __init__(
        ...
        # register potentially trainable weights affecting how each exogenous noise distribution
        # affects each node dimension independently
        ...
        # Create a weight dictionary for edges between nodes
        ...
        # Create a weight dictionary for nodes that are in the same c-component
        self.confounder_means = defaultdict(dict)
        self.confounder_variances = defaultdict(dict)
        for node_a, node_b in confounded_list:
            confounder_mean = torch.tensor(0.0)
            confounder_var = torch.tensor(1.0)
