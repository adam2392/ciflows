...
class LinearGaussianDag(MultidistrCausalFlow):
    def __init__(
        ...
        for node in node_dimensions:
            ...
            self.register_buffer(
                f"exog_mean_{node}_0", noise_means.get(node, torch.tensor(0.0))
            )
            self.register_buffer(
    ...
    def forward(self, batch_size, distr_idx=None, as_dict=False):
        ...
        for node in self.topological_order:
            ...
            if distr_idx == 0 or node not in intervened_vars:
                ...
                node_noise_var = getattr(
                    self, f"exog_variance_{node}_0", torch.tensor(1.0)
                )
            else:
    ...
    def log_prob(self, x, distr_idx=None, return_means_log_vars=False):
        ...
        means_dict = {
            node: torch.zeros_like(dataset[node]).to(device)
            for node in self.topological_order
        }
        log_vars_dict = {
            node: torch.zeros_like(dataset[node]).to(device)
