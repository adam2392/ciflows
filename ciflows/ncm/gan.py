...
from ciflows.ncm.flows import make_flow_model, make_img_flow_model
...
class GAN_NF_NCM(GAN_NCM):
    ...
    def apply_mixing_function(self, v):
        ...
        #     x, _ = flow(x)
        x, _ = self.f_X.inverse_and_log_det(x)
        return x
...
def test_nf_models():
    ...
    hyperparams_1d = {"h-size": 64, "num_glow_steps": 2, "K": 4, "flow_input_shape": (1,)}
    ...
    hyperparams_2d = {"h-size": 64, "num_glow_steps": 2, "K": 4, "flow_input_shape": (3, 32, 32)}
