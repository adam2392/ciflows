    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.linear = self.which_linear(
            self.input_dim, self.arch["in_channels"][0] * (self.bottom_width**2)
        )
                ...
                [
                    GBlock(
                        ...
                        upsample=(
                            ...
                            if self.arch["upsample"][index] and g_index == (self.G_depth - 1)
                            else None
                        ),
                    )
                ]
...
def smoke_test():
    batch_size = 4
    z_dim = 128  # must match G.input_dim
    resolution = 32  # pick 32, 64, 128, etc. to match your training
    fmap = 16  # reduce feature_maps for speed if you like

    # Instantiate generator and discriminator
    G = BigGANDeconv(
        feature_maps=fmap, input_dim=z_dim, resolution=resolution, skip_init=True
    )  # skip_init=True silences the weight‐init printout

    D = BigGANDisc(feature_maps=fmap, resolution=resolution, skip_init=True)  # same here
    ...
    x_fake = G(z)
    print(f" G output (fake images): {x_fake.shape}")
    assert x_fake.shape == (batch_size, 3, resolution, resolution)

    # Discriminator on real and fake
    # (here we just test fake)
    logits = D(x_fake)
    print(f" D output (logits): {logits.shape}")
    assert logits.shape == (batch_size, 1)

    # Quick gradient check
    loss = logits.mean()
    loss.backward()
    print(" ✅ Smoke test passed — forward/backward OK.")
