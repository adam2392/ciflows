...
class ResBlockTranspose(nn.Module):
    def __init__(self, in_channels, out_channels, upsample=False):
        ...
        if in_channels != out_channels or upsample:
            ...
            skip_layers += [nn.Conv2d(in_channels, out_channels, 1), nn.BatchNorm2d(out_channels)]
...
class JointDiscriminator(nn.Module):
    def __init__(
        ...
        self.fc = nn.Sequential(
            spectral_norm(nn.Linear(channels[-1] * final_spatial * final_spatial + z_dim, 512)),
            nn.LeakyReLU(0.2, inplace=True),
            spectral_norm(nn.Linear(512, 128)),
            nn.LeakyReLU(0.2, inplace=True),
            spectral_norm(nn.Linear(128, 1)),  # <— no sigmoid
        )

    def add_instance_noise(self, x):
        if self.use_instance_noise and self.noise_std > 0:
    ...
    def decay_noise(self):
        """Call this each epoch to slowly reduce noise"""
