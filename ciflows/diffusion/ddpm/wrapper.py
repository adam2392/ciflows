...
class DDPMWrapper(pl.LightningModule):
    def __init__(
        ...
        assert eval_mode in ["sample", "recons", "vae_sample", "vae_recons"]
    ...
    def forward(
        ...
        spaced_nw = (
            SpacedDiffusionForm2 if isinstance(self.online_network, DDPMv2) else SpacedDiffusion
        )
    ...
    def training_step(self, batch, batch_idx):
        ...
        torch.nn.utils.clip_grad_norm_(self.online_network.decoder.parameters(), self.grad_clip_val)
        optim.step()
    ...
    def predict_step(self, batch, batch_idx, dataloader_idx=None):
        if not self.conditional:
            if self.guidance_weight != 0.0:
                raise ValueError("Guidance weight cannot be non-zero when using unconditional DDPM")
            x_t = batch
            return self(
                x_t,
                cond=None,
        ...
        if self.eval_mode == "sample":
            ...
            x_t = self.online_network.compute_noisy_input(
                ...
                torch.tensor([self.online_network.T - 1] * img.size(0), device=img.device),
            )

            if isinstance(self.online_network, DDPMv2):
                x_t += recons

        elif self.eval_mode == "vae_sample":
            x_t, recons = batch
            recons = 2 * recons - 1
            x_t = x_t * self.temp
            # Formulation-2 initial latent
